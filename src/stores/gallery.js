...
export const useGalleryStore = defineStore("gallery", () => {
  ...
  const itemsByType = computed(() => {
    ...
    Object.values(GALLERY_MEDIA_TYPES).forEach((type) => {
      grouped[type] = filteredItems.value.filter(
  ...
  const itemsByCategory = computed(() => {
    ...
    Object.values(GALLERY_CATEGORIES).forEach((category) => {
      grouped[category] = filteredItems.value.filter(
        (item) => item.mediaCategory === category
      );
    });
    // Add items without category
    grouped.Other = filteredItems.value.filter(
      (item) =>
        !item.mediaCategory ||
        !Object.values(GALLERY_CATEGORIES).includes(item.mediaCategory)
    );
    return grouped;
  });
  ...
  const isAllSelected = computed(
    () =>
      ...
      selectedItemsCount.value === filteredItems.value.length
  );
  ...
  const loadGallery = async (nodeId = null, params = {}) => {
    ...
    try {
      ...
      const response = await galleryService.getNodeGallery(
        currentNodeId.value,
        queryParams
      );
  ...
  const loadGallerySummary = async (nodeId = null) => {
    ...
    try {
      const response = await galleryService.getGallerySummary(
        currentNodeId.value
      );
  ...
  const addGalleryItem = (item) => {
    const existingIndex = galleryItems.value.findIndex(
      (i) => i.recCode === item.recCode
    );
    if (existingIndex >= 0) {
  ...
  const updateGalleryItem = async (itemId, updateData) => {
    try {
      const response = await galleryService.updateMediaDetails(
        itemId,
        updateData
      );
  ...
  const uploadFiles = async (files, uploadData, options = {}) => {
    ...
    try {
      for (const uploadItem of uploadItems) {
        uploadProgress.value.currentFile = uploadItem.file.name;

        // Use the service to upload with metadata
  ...
  const bulkDelete = async (itemIds) => {
    try {
      ...
      galleryItems.value = galleryItems.value.filter(
        (item) => !itemIds.includes(item.recCode)
      );
  ...
  const reorderItems = async (orderedItemIds) => {
    try {
      await galleryService.updateMediaOrder(
        currentNodeId.value,
        orderedItemIds
      );
  ...
  const downloadItem = async (item) => {
    try {
      await galleryService.downloadMedia(
        item.recCode,
  ...
  const getSelectedItems = () => {
    return galleryItems.value.filter((item) =>
      selectedItems.value.includes(item.recCode)
    );
  };
  ...
  const getCameraUploadStats = () => {
    const cameraUploads = galleryItems.value.filter(
      (item) =>
    ...
    return {
      ...
      byDirection: {
        front: cameraUploads.filter(
          (item) => item.metadata?.cameraDirection === "front"
        ).length,
        rear: cameraUploads.filter(
          (item) => item.metadata?.cameraDirection === "rear"
        ).length,
      },
      byCategory: Object.values(GALLERY_CATEGORIES).reduce((acc, category) => {
        acc[category] = cameraUploads.filter(
          (item) => item.mediaCategory === category
        ).length;
        return acc;
      }, {}),
      recent: cameraUploads.filter((item) => {
